...
class InputsControlWrapper(gym.Wrapper):
    ...
    def get_gamepad_action(self):
        ...
        # Check if intervention should be active from controller or our override flag
        intervention_is_active = self.controller.should_intervene()
        
        # If we've set the intervention_active flag, override the controller's value
        if hasattr(self, 'intervention_active'):
            intervention_is_active = intervention_is_active or self.intervention_active
    ...
    def reset(self, **kwargs):
        ...
        # Make sure intervention is active after reset
        if hasattr(self.controller, 'key_states'):
            self.controller.key_states['intervention'] = True
        self.intervention_active = True
        return self.env.reset(**kwargs)
        
    def force_intervention(self, active=True):
        """Force intervention mode to be active or inactive."""
        if hasattr(self.controller, 'key_states'):
            self.controller.key_states['intervention'] = active
...
class ResetDelayWrapper(gym.Wrapper):
    """
    Wrapper that adds a time delay on reset.
    Prevents automatic movement after reset by ensuring intervention mode stays active.
    After the first reset, prevents any further resets to keep the arm in manual control.
    """

    def __init__(self, env, delay_seconds=600):
        """
        ...
        self.has_reset_once = False  # Track if we've already reset once
        # Default to manual control mode to prevent auto-movement
        self.force_manual_control = True
        # Store the last observation and info to return for subsequent resets
        self.last_observation = None
        self.last_info = {}

    def reset(self, **kwargs):
        """Reset the environment with a time delay."""
        if self.has_reset_once and self.last_observation is not None:
            # If we've already reset once, don't actually reset
            logging.info("Preventing reset since we've already reset once")
            # Return the stored observation without resetting
            # This effectively disables resets after the first one
            return self.last_observation, self.last_info
        
        # First reset - apply the delay
        logging.info("Performing initial reset with delay")
        forced_delay = 3.0  # Short delay for first reset
        logging.info(f"Reset delay of {forced_delay} seconds")
        time.sleep(forced_delay)
        
        # Mark that we've reset once
        self.has_reset_once = True
        
        # Call the parent reset method
        obs, info = self.env.reset(**kwargs)
        
        # Store the observation and info for future resets
        self.last_observation = obs
        self.last_info = info
        
        # Force intervention mode to be active in all wrappers
        # This prevents automatic movement after reset
        env = self.env
        while hasattr(env, 'env'):
            # Check for our InputsControlWrapper with the force_intervention method
            if hasattr(env, 'force_intervention'):
                env.force_intervention(True)
                print("Forcing intervention mode to be active")
                break
            # Check for KeyboardControlWrapper
            elif hasattr(env, 'is_intervention_active'):
                env.is_intervention_active = True
                print("Setting is_intervention_active to True")
                break
            # Check for controller with key_states
            elif hasattr(env, 'controller') and hasattr(env.controller, 'key_states'):
                env.controller.key_states['intervention'] = True
                print("Setting controller intervention flag to True")
                break
            env = env.env
